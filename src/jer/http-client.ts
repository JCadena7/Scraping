...
export interface JerHttpClientOptions { timeoutMs: number; delayMs: number; maxRetries: number; userAgent: string; fetchImpl?: typeof fetch; sleepImpl?: (ms: number, signal?: AbortSignal) => Promise<void>; logger?: (event: Record<string, unknown>) => void; }
...
export class JerHttpClient {
  ...
  constructor(private readonly options: JerHttpClientOptions) { this.fetchImpl = options.fetchImpl ?? fetch; this.sleepImpl = options.sleepImpl ?? sleep; }
    ...
    try {
      ...
      for (let attempt = 1; attempt <= this.options.maxRetries + 1; attempt++) {
        ...
        try {
          ...
          await this.sleepImpl(retryAfterMs ?? backoff(attempt), signal); throwIfAborted(signal);
        } catch (error) {
          ...
          await this.sleepImpl(backoff(attempt), signal); throwIfAborted(signal);
